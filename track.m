function [trackResults, channel,outData]= track(fid, channel, settings,data)
...
%% Find FLL error or  PLL error and update carrier NCO ----------------------------------
            if step_cnt<=K_step 
                ...
                %四象限反正切鉴别
                carrError = atan2(cross,dot) / (2*pi*T);
                past_I_accum = I_P;
                past_Q_accum = Q_P;   
...
%% Record various measures to show in postprocessing ----------------------
            ...
            trackResults(channelNr).absoluteSample(loopCnt) = fid - blksize;
            ...
            outData = ...
                [outData promptCode .* iBasebandSignal + 1j*promptCode .* qBasebandSignal];
        end % for loopCnt
    ...
    %trackResults(channelNr).CarrNcos = CarrNcos;
    %trackResults(channelNr).codeNcos = codeNcos;   
end % for channelNr 
